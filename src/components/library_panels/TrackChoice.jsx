...
import { Text, Subtitle } from '../../styles/StyledComponents';
import { secondaryColor } from '../../utils/DisplaySettings';

// Demo tracks configuration
const DEMO_TRACKS = [
    {
        filename: 'ES_No More - dasloe.mp3',
        title: 'No More',
        artist: 'Dasloe'
    },
    {
...
/** Initial `{ name, url, isDemo }` for DemoPlayerPage — mirrors first entry in `DEMO_TRACKS`. */
export function getDefaultDemoTrackPayload() {
    const track = DEMO_TRACKS[0];
    return {
        name: `${track.title} - ${track.artist}`,
        url: buildDemoAudioUrl(track.filename),
        isDemo: true
    };
}
