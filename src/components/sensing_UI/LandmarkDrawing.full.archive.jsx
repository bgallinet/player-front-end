/**
 * ARCHIVED — full MediaPipe face mesh canvas drawer (replaced by `LandmarkDrawing.js` + `reactionAnalyticsLandmarkConfig.js`).
 * Kept for reference / design comparison only; not imported by the app.
 */
import {
    primaryColor,
    secondaryColor,
...
/**
 * Canvas utilities for MediaPipe face mesh drawing (static methods on LandmarkDrawing).
 * Status strings (smiling, jawOpen, nodding, noddingSmiling, noddingJawOpen) adjust colors;
 * see getColorForStatus for per-region behavior.
 *
 * Use drawLandmarksOnCanvas for the holistic overlay path: it maps reaction frameData → status,
 * normalizes flat [x,y,...] landmark buffers, then calls drawAllLandmarks.
 */
...
/**
 * Map reaction frame data to a status string for getColorForStatus.
...
/**
 * Draw face landmarks on canvas from holistic-style inputs (reaction coloring + flat or object buffers).
